...
imputation:
  ...
  # The engine runs microimpute.Imputer (regime gate + chained QRF) per step.
...
- **passthrough vs synthesize** is a per-variable tag, declared. This is what's left of the 1,300-line `VariableSemanticSpec` — the rest dissolves because `microimpute.Imputer` now does regime+chaining generically.
...
3. **`SpineBuilder`** — implement §4 (split base; one half kept; other half stripped to declared columns). Generic; no country logic.
4. **`ImputationRunner`** — for each `imputation` step, fit `microimpute.Imputer` on the donor (weighted) over the step's var block, conditioned on `condition_on` (+ chain), and apply to the target half. This is the heart; microimpute already does the model. The runner just orchestrates the declared graph + entity grain.
...
| --- | --- |
| `pipelines/us.py` spine construction (~split/clone) | `microplex.SpineBuilder` (generic), driven by `spine:` spec |
| `pipelines/us.py` donor integration + `donor_imputers.py` | `microplex.ImputationRunner` + `microimpute.Imputer` (done), driven by `imputation:` spec |
...
- `synthetic_puf` half: **strip to demographics (+ tax_unit_id)**, then **synthesize the whole PUF tax-unit** (wages included) through `microimpute.Imputer` conditioned on demographics + the chain. Wide distributions on the first chained variables are correct — it's a barely-conditioned pure synthetic PUF. **Never keep/condition on CPS wages here.**
