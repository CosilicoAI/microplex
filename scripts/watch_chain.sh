...
steps0=$(grep -c "=== CHAIN" $LOG 2>/dev/null || echo 0)
tb0=$(grep -c "Traceback" $LOG 2>/dev/null || echo 0)
fail0=$(grep -c "passed=False" $LOG 2>/dev/null || echo 0)
...
while true; do
  ...
  steps=$(grep -c "=== CHAIN" $LOG 2>/dev/null || echo 0)
  tb=$(grep -c "Traceback" $LOG 2>/dev/null || echo 0)
  fail=$(grep -c "passed=False" $LOG 2>/dev/null || echo 0)
  if (( tb > tb0 )); then
